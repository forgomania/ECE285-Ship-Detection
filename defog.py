...
def getV1(m, r, eps, w, maxV1):
    ...
    V1 = guidedfilter(V1, zmMinFilterGray(V1, 7), r, eps)  # optimize by using guided filter
    bins = 2000
    ...
    V1 = np.minimum(V1 * w, maxV1)  # constrain the range of value
    ...
    if lines is not None:
        for i in range(len(lines)):
                for x1, y1,x2,y2 in lines[i]:
                    cv2.line(m, (x1, y1), (x2, y2), (0, 0, 255))
    return m